#!/usr/bin/env python3
"""Preflight validator for the Fly House v0.3 environment package.

The validator is intentionally Blender-free. It protects the reference-driven scene
contract, collision/observer semantics and asset provenance before a Blender build is
started. Use ``--strict-assets`` for a visual-review candidate.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate Fly House v0.3 inputs before Blender build")
    parser.add_argument(
        "--strict-assets",
        action="store_true",
        help="fail if a visual-approval asset slot is incomplete or its file is missing",
    )
...
def validate(strict_assets: bool) -> dict:
    ...
    if spec.get("stage") != "reference-detail-v0.3":
        warnings.append(f"expected reference-detail-v0.3 stage, got {spec.get('stage')!r}")
    if "Z-up" not in str(spec.get("coordinateSystem", "")):
        errors.append("Fly House v0.3 coordinateSystem must explicitly remain Z-up")
    ...
    if not _vec3(observer.get("spawn")):
        errors.append("observer.spawn must be a numeric vec3")
    if observer.get("debugNoclipKey") != "KeyN":
        warnings.append("observer debug noclip should remain an explicit KeyN-only debugging escape hatch")
    ...
    required_gates = (
        ...
        "mustKeepObserverCollisionEnabledByDefault",
        "mustKeepObserverInvisibleToFlySensors",
        "mustKeepBrowserAndBlenderCoordinateSystemsAligned",
    )
    for key in required_gates:
        if gates.get(key) is not True:
            errors.append(f"qualityGates.{key} must remain true")

    bounds = spec.get("worldBounds", {})
    if not _vec3(bounds.get("min")) or not _vec3(bounds.get("max")):
        errors.append("worldBounds.min/max must be numeric vec3 values")
    ...
    for camera in cameras:
        cid = camera.get("id", "<unnamed>")
        if not _vec3(camera.get("position")) or not _vec3(camera.get("lookAt")):
            errors.append(f"preview camera {cid!r} must define position/lookAt vec3 values")
        if not isinstance(camera.get("focalLengthMm"), (int, float)) or camera.get("focalLengthMm", 0) <= 0:
            errors.append(f"preview camera {cid!r} must define positive focalLengthMm")

    rooms = spec.get("rooms", [])
    room_ids = [room.get("id") for room in rooms]
    if len(rooms) < 2:
        errors.append("Fly House must retain the main room and connected bedroom")
    ...
    for room in rooms:
        rid = room.get("id", "<unnamed>")
        if not _vec3(room.get("origin")) or not _vec3(room.get("size")):
            errors.append(f"room {rid!r} origin/size must be numeric vec3 values")
        elif any(v <= 0 for v in room["size"]):
            errors.append(f"room {rid!r} size values must be positive")

    main_room = next((room for room in rooms if room.get("id") == "main-room"), None)
    if main_room and (main_room.get("size", [0, 0, 0])[0] < 8.3 or main_room.get("size", [0, 0, 0])[1] < 6.3):
        errors.append("main-room must preserve the enlarged v0.3 footprint (about 8.4 x 6.4 m)")
    ...
    for hero in heroes:
        ...
        for field in ("position", "rotationDeg", "size"):
            if not _vec3(hero.get(field)):
                errors.append(f"hero {hero_id!r}.{field} must contain three numbers")
        size = hero.get("size")
        if _vec3(size) and any(value <= 0 for value in size):
    ...
    for slot in slots:
        ...
        missing_source_fields = [key for key in ("url", "author", "license", "retrievedAt") if not source.get(key)]
        if missing_source_fields:
            errors.append(f"asset slot {slot_id!r} source is missing: {', '.join(missing_source_fields)}")

        license_id = source.get("license")
        if license_id and license_id not in allowed_licenses:
            errors.append(f"asset slot {slot_id!r} uses disallowed/unrecognized license {license_id!r}")
        ...
        if not under(resolved, asset_root):
            ...
            errors.append(f"asset slot {slot_id!r} uses unsupported format {resolved.suffix or '<none>'}")
    ...
    result = {
        ...
        "summary": {
            ...
            "observerCollision": observer.get("collides"),
        },
...
def print_human(result: dict) -> None:
    ...
    print("Fly House v0.3 preflight")
    print("------------------------")
    if summary:
        print(
            "stage={stage} rooms={rooms} heroes={heroObjects} assets={configuredAssetSlots}/{assetSlots} "
            "required={requiredAssetsReady}/{requiredAssetSlots} cameras={benchmarkCameras} collision={observerCollision}".format(**summary)
        )
    ...
    if result.get("ok"):
        if result.get("visualApprovalReady"):
            ...
            print("OK: project structure is valid; visual approval is still blocked by missing required assets.")
    else:
