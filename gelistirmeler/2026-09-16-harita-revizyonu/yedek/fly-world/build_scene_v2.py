...
def architecture(m):
    ...
    for y in (-3.08,3.08): box(f"skirting-{y}",(8.05,.08,.16),(0,y,.12),m["wood2"],c)
    return c
...
def reference_dressing(root, manifest, m):
    ...
    box("window-glass",(.035,1.92,1.50),(-4.12,-.72,1.49),m["glass"],c,collision=True)
    for i,y in enumerate((-1.53,-1.15,-.77,-.39,-.01)):
        box(f"window-bar-v-{i}",(.055,.045,1.50),(-4.05,y,1.49),m["metal"],c,collision=True)
    for i,z in enumerate((.84,1.28,1.72,2.14)):
        box(f"window-bar-h-{i}",(.055,1.92,.045),(-4.05,-.72,z),m["metal"],c,collision=True)
    box("curtain-rail",(.12,2.25,.10),(-3.87,-.66,2.67),m["wood2"],c)
    ...
    sp=data.splines.new("BEZIER"); pts=((2.05,.76,1.20),(2.05,.76,2.35),(1.35,.76,2.56),(-2.35,.76,2.56),(-2.85,.76,2.42)); sp.bezier_points.add(len(pts)-1)
    ...
    box("stove-fire-window",(.50,.025,.29),(2.05,.32,.61),m["fire"],c)
    ...
    box("blue-bag",(.76,.36,.55),(.55,.18,.31),m["blue"],c,bevel=.07,collision=True,rot=(0,0,-.16))
    ...
    box("bed-headboard",(1.52,.17,1.15),(6.72,2.92,1.12),m["wood"],c,bevel=.035,collision=True)
    box("bedroom-bookshelf",(1.10,.34,1.62),(7.65,3.18,.81),m["wood"],c,bevel=.025,collision=True)
    for row in range(3):
        for i in range(5):
            box(f"book-{row}-{i}",(.10+.03*(i%2),.22,.24+.04*((i+row)%2)),(7.28+i*.16,2.96,.32+row*.46),(m["red"],m["green"],m["wood3"],m["blue"])[(i+row)%4],c)
    ...
    box("bedroom-window",(1.34,.035,1.40),(5.61,3.58,1.48),m["glass"],c,collision=True)
    for i in range(4): box(f"bed-curtain-{i}",(.28,.055,1.78),(4.84+i*.25,3.48,1.55),m["red"],c)
...
def lighting(m):
    ...
    point("stove-glow",(2.05,.22,.62),160,(1,.12,.012),.58)
