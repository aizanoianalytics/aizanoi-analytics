...
export function installEvidenceMode({ runtime, city = {}, root = document.body } = {}) {
  ...
  panel.innerHTML = `
    <h2>Research Lens</h2>
    <p>Evidence status describes the reconstruction claim, not visual quality. “Plausible” legacy records are grouped as inferred.</p>
    <div class="awResearchLegend">
      ${EVIDENCE_MODE_ORDER.map((id) => {
        const item = EVIDENCE_LEVELS[id];
        return `<div data-evidence-group="${id}"><i class="awEvidenceDot" style="background:${item.color}"></i><b>${esc(item.short)}</b><span>${counts[id] || 0}</span></div>`;
      }).join('')}
    </div>
  ...
  function update() {
    ...
    if (focus && focusNode) {
      ...
      focusNode.innerHTML = `<b>${esc(focus.record.name || focus.record.id)} · ${esc(display.short)}</b><small>${esc(raw.note || focus.record.detail || focus.record.note || `${Math.round(focus.distance)} m away`)}</small>`;
    }
    if (nearbyNode) {
      nearbyNode.innerHTML = nearby.map(({ record, distance }) => {
        const { display } = recordEvidence(record);
        return `<button type="button" data-aw-evidence-visit="${esc(record.id)}"><i class="awEvidenceDot" style="background:${display.color}"></i><strong>${esc(record.name || record.id)}</strong><small>${esc(display.short)} · ${Math.round(distance)} m</small></button>`;
