...
export class FlySimulation {
  constructor(environment,{fixedDt=.02,gravity=null,rngState=0,downDirection=environment?.downDirection??new Vec3(0,-1,0)}={}){const authored=environment?.meta?.artifactHashes;if(!environment?.hash||!environment?.glbHash||!environment?.schemaVersion||authored?.environmentSource!==environment.hash||authored?.flyHouseGlb!==environment.glbHash)throw new TypeError('environment authored artifact hashes and schema required');this.environment=environment;this.fixedDt=fixedDt;this.downDirection=asVec(downDirection).normalize();this.upDirection=this.downDirection.mul(-1);this.gravity=gravity==null?this.downDirection.mul(9.81):asVec(gravity);this.physicsCollisionAdapter=new AuthoredSurfaceAdapter(environment.surfaces??[],environment.raycast);this.registry=new Map();this.time=0;this.tick=0;this.paused=false;this.rngState={seed:rngState};this.environmentState=environment.dynamicState??{};this.provenance=Object.freeze({physics:MODELLED,collision:Object.freeze({...MODELLED,units:'authored surface/raycast'}),sensors:MODELLED})}
  addFly({flyId,body=new BodyState(),provenance=MODELLED}){if(!flyId||this.registry.has(flyId))throw new Error('flyId must be unique');const fly={flyId,body:cloneBody(body),motors:{thrust:0,pitch:0,yaw:0,roll:0},room:null,zones:[],contact:{phase:'AIRBORNE',grounded:false,surfaceId:null,normal:null},sensors:null,sensorHistory:[],provenance:validateProvenance(provenance)};fly.room=this.environment.roomAt?.(fly.body.position)??null;fly.zones=this.environment.zonesAt?.(fly.body.position)??[];this.registry.set(flyId,fly);this.#sense(fly)}
...
export function checkpoint(sim){return cloneJson({version:'checkpoint-1',environmentHash:sim.environment.hash,glbHash:sim.environment.glbHash??null,environmentSchema:sim.environment.schemaVersion,time:sim.time,tick:sim.tick,rngState:sim.rngState,sensorHistory:[...sim.registry.values()].map(f=>({flyId:f.flyId,history:f.sensorHistory})),environmentState:sim.environmentState,flies:[...sim.registry.values()].map(f=>({flyId:f.flyId,body:f.body.toJSON(),motors:f.motors,room:f.room,zones:f.zones,contact:f.contact,provenance:f.provenance}))})}
