...
export class Vec3 {
  ...
  clone(){return new Vec3(this.x,this.y,this.z)} add(v){return new Vec3(this.x+v.x,this.y+v.y,this.z+v.z)} sub(v){return new Vec3(this.x-v.x,this.y-v.y,this.z-v.z)} mul(s){return new Vec3(this.x*s,this.y*s,this.z*s)} dot(v){return this.x*v.x+this.y*v.y+this.z*v.z} length(){return Math.hypot(this.x,this.y,this.z)} normalize(){const l=this.length();return l?this.mul(1/l):new Vec3()}
...
export class AuthoredSurfaceAdapter {
  constructor(surfaces=[],delegate=null){this.surfaces=surfaces.map(s=>({...s,point:asVec(s.point),normal:asVec(s.normal).normalize()}));this.delegate=delegate}
...
export class FlySimulation {
  constructor(environment,{fixedDt=.02,gravity=null,rngState=0,downDirection=environment?.downDirection??new Vec3(0,-1,0),controller=null,controllerFactory=null,motorLimits={}}={}){const authored=environment?.meta?.artifactHashes;if(!environment?.hash||!environment?.glbHash||!environment?.schemaVersion||authored?.environmentSource!==environment.hash||authored?.flyHouseGlb!==environment.glbHash)throw new TypeError('environment authored artifact hashes and schema required');this.environment=environment;this.fixedDt=fixedDt;this.downDirection=asVec(downDirection).normalize();this.upDirection=this.downDirection.mul(-1);this.gravity=gravity==null?this.downDirection.mul(9.81):asVec(gravity);this.motorLimits={thrust:20,pitch:.02,yaw:.02,roll:.02,...motorLimits};this.physicsCollisionAdapter=new AuthoredSurfaceAdapter(environment.surfaces??[],environment.raycast);this.controller=controller;this.controllerFactory=controllerFactory;this.registry=new Map();this.time=0;this.tick=0;this.paused=false;this.rngState={seed:rngState};this.environmentState=environment.dynamicState??{};this.provenance=Object.freeze({physics:MODELLED,collision:Object.freeze({...MODELLED,units:'authored surface/raycast'}),sensors:MODELLED})}
  ...
  stepOne(dt=this.fixedDt){
    ...
    for(const f of this.registry.values()){
      this.#sense(f);
      if(f.controller&&typeof f.controller.step==='function') f.motors=normalizeMotors(f.controller.step(f.sensors,f.controllerState,{dt,tick:this.tick,flyId:f.flyId}),this.motorLimits);
...
export class HeuristicBaselineController {constructor(){this.name='HEURISTIC BASELINE CONTROLLER';this.provenance='HEURISTIC'}step(sensor){const grounded=sensor.contact?.grounded;const odor=Number(sensor.channels?.olfaction?.value??0);return {thrust:grounded?0.000018:0.000012+odor*0.000002,pitch:odor>0?Math.min(0.02,odor*0.006):0,yaw:0,roll:0}}}
export class FlyWireLC4EscapeController {
  constructor(graph){if(!graph?.provenance||graph.provenance.label!=='CONNECTOME-DERIVED'||!Array.isArray(graph.edges))throw new TypeError('FlyWire graph provenance required');this.graph=graph;this.name='FLYWIRE FAFB V783 LC4 ESCAPE SUBGRAPH';this.provenance='CONNECTOME-DERIVED';this.lc4ToDn=graph.edges.filter((edge)=>edge.pre_type==='LC4'&&['DNp02','DNp11'].includes(edge.post_type));this.totalSynapses=this.lc4ToDn.reduce((sum,edge)=>sum+edge.syn_count,0)}
  step(sensor){const light=Number(sensor.channels?.light?.value??0);const visualDrive=Math.max(0,Math.min(1,light));const dnDrive=visualDrive*Math.min(1,this.totalSynapses/2000);return {thrust:sensor.contact?.grounded?0.000018:0.000012+dnDrive*0.00001,pitch:dnDrive*0.02,yaw:0,roll:0,state:{visualDrive,dnDrive,edgeCount:this.lc4ToDn.length,totalSynapses:this.totalSynapses,transduction:'MODELLED'}}}
}
const telemetrySensorSummary=(value)=>{if(value==null)return null;if(typeof value!=='object'||typeof value.version!=='string'||typeof value.channels!=='object')throw new TypeError('telemetry sensors allowlist');const channels={};for(const [name,sensor] of Object.entries(value.channels)){if(!/^[a-z]+$/.test(name)||!sensor||typeof sensor.status!=='string')throw new TypeError('telemetry sensor channel');const raw=sensor.value;const safe=Array.isArray(raw)?finiteArray(raw,3,`sensor ${name}`):Number.isFinite(raw)?raw:raw==null?null:undefined;if(safe===undefined)throw new TypeError('telemetry sensor value');channels[name]={status:sensor.status,value:safe,units:String(sensor.units??'n/a')}}return {version:value.version,channels}}
...
export class TelemetryProtocol {encode(frame){if(!frame||Object.keys(frame).some(k=>!telemetryKeys.has(k)))throw new TypeError('telemetry allowlist');if(typeof frame.flyId!=='string'||!frame.flyId)throw new TypeError('telemetry flyId required');if(!Number.isInteger(frame.sequence)||frame.sequence<0)throw new TypeError('telemetry sequence required');const identity=frame.identity;if(!identity||typeof identity.environmentHash!=='string'||!identity.environmentHash||typeof identity.glbHash!=='string'||!identity.glbHash||Object.keys(identity).some(k=>!['environmentHash','glbHash'].includes(k)))throw new TypeError('telemetry identity required');const state=frame.state??{};if(Object.keys(state).some(k=>!['room','position','orientation','contact','sensors','motor'].includes(k)))throw new TypeError('telemetry state allowlist');const position=state.position==null?null:finiteArray(state.position,3,'position');const orientation=state.orientation==null?[0,0,0,1]:finiteArray(state.orientation,4,'orientation');const motor=state.motor==null?null:finiteArray([state.motor.thrust,state.motor.pitch,state.motor.yaw,state.motor.roll],4,'motor');return JSON.stringify({version:'telemetry-1',flyId:frame.flyId,sequence:frame.sequence,identity:{environmentHash:identity.environmentHash,glbHash:identity.glbHash},state:{room:state.room??null,position,orientation,contact:telemetryContact(state.contact),sensors:telemetrySensorSummary(state.sensors),motor},metadata:{controller:frame.metadata?.controller??'HEURISTIC TEST CONTROLLER',provenance:frame.metadata?.provenance??'MODELLED',units:frame.metadata?.units??'SI'},lag:frame.lag??0})}}
...
export function checkpoint(sim){return cloneJson({version:'checkpoint-1',environmentHash:sim.environment.hash,glbHash:sim.environment.glbHash??null,environmentSchema:sim.environment.schemaVersion,time:sim.time,tick:sim.tick,rngState:sim.rngState,sensorHistory:[...sim.registry.values()].map(f=>({flyId:f.flyId,history:f.sensorHistory})),environmentState:sim.environmentState,flies:[...sim.registry.values()].map(f=>({flyId:f.flyId,body:f.body.toJSON(),motors:f.motors,controller:f.controller?.name??null,controllerState:f.controllerState,room:f.room,zones:f.zones,contact:f.contact,provenance:f.provenance}))})}
export function restore(sim,cp){if(cp.version!=='checkpoint-1')throw new Error('checkpoint version');if(cp.environmentHash!==sim.environment.hash)throw new Error('environment hash mismatch');if((cp.glbHash??null)!==(sim.environment.glbHash??null))throw new Error('GLB hash mismatch');if(cp.environmentSchema!==sim.environment.schemaVersion)throw new Error('environment schema mismatch');sim.registry.clear();for(const f of cp.flies)sim.addFly({flyId:f.flyId,body:new BodyState(f.body),provenance:f.provenance});for(const f of cp.flies){const live=sim.getFly(f.flyId);live.motors=f.motors;live.controllerState=f.controllerState??{};live.room=f.room;live.zones=f.zones;live.contact=f.contact;live.sensorHistory=cp.sensorHistory?.find(x=>x.flyId===f.flyId)?.history??[];live.sensors=live.sensorHistory.at(-1)??live.sensors}sim.time=cp.time;sim.tick=cp.tick??Math.round(sim.time/sim.fixedDt);sim.rngState=cp.rngState;sim.environmentState=cp.environmentState;return sim}
...
export function createFlyWorldEnvironmentAdapter(environment,identity={}){
  ...
  return Object.freeze({
    ...
    meta:Object.freeze({artifactHashes:Object.freeze({environmentSource:String(hash),flyHouseGlb:String(glbHash)})}),
