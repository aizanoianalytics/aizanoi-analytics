...
const visionSample=(environment,position)=>{const maxRange=.5;const distances=VISION_DIRECTIONS.map(direction=>{const hit=environment.raycast?.(position,direction,maxRange);return hit?Math.max(0,Math.min(1,hit.distance/maxRange)):1});const minDistance=Math.min(...distances)*maxRange;return {status:'MODELLED',value:distances,looming:Math.max(0,Math.min(1,1-minDistance/.5)),units:'normalized distance',provenance:Object.freeze({label:'BIOLOGICALLY CONSTRAINED + MODELLED',source:'authoritative directional ray sampling',units:'normalized distance',calibrated:false,assumptions:['eight horizontal directions','ray occupancy stands in for visual contrast and looming'],limitations:['not ommatidial retina','no image formation or optic-flow field'],version:'vision-v1',sourceReferences:['Drosophila visual field literature; authored Fly House collision artifact']})};};
 export class FlySimulation {
  ...
  stepOne(dt=this.fixedDt){
    ...
    for(const f of this.registry.values()){
      this.#sense(f);
      if(f.controller&&typeof f.controller.step==='function'){
...
const telemetrySensorSummary=(value)=>{if(value==null)return null;if(typeof value!=='object'||typeof value.version!=='string'||typeof value.channels!=='object')throw new TypeError('telemetry sensors allowlist');const channels={};for(const [name,sensor] of Object.entries(value.channels)){if(!/^[a-z]+$/.test(name)||!sensor||typeof sensor.status!=='string')throw new TypeError('telemetry sensor channel');const raw=sensor.value;const safe=Array.isArray(raw)?finiteArray(raw,name==='vision'?8:3,`sensor ${name}`):Number.isFinite(raw)?raw:raw==null?null:undefined;if(safe===undefined)throw new TypeError('telemetry sensor value');channels[name]={status:sensor.status,value:safe,looming:Number.isFinite(sensor.looming)?sensor.looming:null,units:String(sensor.units??'n/a')}}return {version:value.version,channels}}
...
export function checkpoint(sim){return cloneJson({version:'checkpoint-2',environmentHash:sim.environment.hash,glbHash:sim.environment.glbHash??null,environmentSchema:sim.environment.schemaVersion,artifactIdentity:sim.environment.meta?.artifactHashes??{},physicsArtifactSchema:sim.environment.meta?.artifactHashes?.physicsSchema??null,time:sim.time,tick:sim.tick,rngState:sim.rngState,sensorHistory:[...sim.registry.values()].map(f=>({flyId:f.flyId,history:f.sensorHistory})),environmentState:sim.environmentState,flies:[...sim.registry.values()].map(f=>({flyId:f.flyId,body:f.body.toJSON(),motors:f.motors,controller:f.controller?.name??null,controllerVersion:f.controller?.version??'unversioned',controllerProvenance:f.controller?.provenance??f.provenance?.label??null,controllerState:f.controllerState,room:f.room,zones:f.zones,contact:f.contact,provenance:f.provenance}))})}
