...
export class TelemetryProtocol {encode(frame){if(!frame||Object.keys(frame).some(k=>!telemetryKeys.has(k)))throw new TypeError('telemetry allowlist');if(typeof frame.flyId!=='string'||!frame.flyId)throw new TypeError('telemetry flyId required');if(!Number.isInteger(frame.sequence)||frame.sequence<0)throw new TypeError('telemetry sequence required');const identity=frame.identity;if(!identity||typeof identity.environmentHash!=='string'||!identity.environmentHash||typeof identity.glbHash!=='string'||!identity.glbHash||Object.keys(identity).some(k=>!['environmentHash','glbHash'].includes(k)))throw new TypeError('telemetry identity required');const state=frame.state??{};if(Object.keys(state).some(k=>!['room','position','orientation','contact'].includes(k)))throw new TypeError('telemetry state allowlist');const position=state.position==null?null:finiteArray(state.position,3,'position');const orientation=state.orientation==null?[0,0,0,1]:finiteArray(state.orientation,4,'orientation');return JSON.stringify({version:'telemetry-1',flyId:frame.flyId,sequence:frame.sequence,identity:{environmentHash:identity.environmentHash,glbHash:identity.glbHash},state:{room:state.room??null,position,orientation,contact:telemetryContact(state.contact)},metadata:{controller:frame.metadata?.controller??'HEURISTIC TEST CONTROLLER',provenance:frame.metadata?.provenance??'MODELLED',units:frame.metadata?.units??'SI'},lag:frame.lag??0})}}
