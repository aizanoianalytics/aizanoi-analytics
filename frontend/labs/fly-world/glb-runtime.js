...
scene.fog = new THREE.Fog(0x28221b, 14, 36);
...
function collectCollisionRoots(root) {
  ...
  // Blender extras are exported as userData. Prefer explicit metadata, then use
  // conservative naming fallbacks for older generated hero assets.
  root.traverse((object) => {
    const explicit = object.userData?.collision === 'solid';
    const byName = /^(WALL__|CEILING__|window-glass|bedroom-window|ASSET__)/.test(object.name || '');
    if (explicit || byName) addCollider(object);
  });
}
...
class Observer {
  ...
  syncStatus() {
    if (statusEl) statusEl.textContent = this.noClip ? 'Blender house · DEBUG NOCLIP' : 'Blender house · collision ON';
  }

  reset() {
    camera.position.set(-1.55, -1.85, 1.58);
    this.yaw = .78;
    this.pitch = -.06;
  }
...
function lighting() {
  scene.add(new THREE.HemisphereLight(0xb9d2e1, 0x4a3426, 1.04));
