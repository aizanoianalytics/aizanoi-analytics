...
const MATERIALS = {
  ...
  proxy: new THREE.MeshStandardMaterial({ color: 0xa77b56, roughness: 0.82 }),
...
function cylinder(size, position, material, name, radialSegments = 24) {
  const radius = Math.max(size[0], size[1]) * 0.5;
  const mesh = new THREE.Mesh(new THREE.CylinderGeometry(radius, radius, size[2], radialSegments), material);
  mesh.position.set(position[0], position[2] * 0.5 + position[2] - position[2] * 0.5, position[1]);
  mesh.name = name;
  mesh.castShadow = true;
  mesh.receiveShadow = true;
  scene.add(mesh);
...
function buildMainRoom() {
  ...
  // West wall around the reference window.
  ...
  // East wall around bedroom doorway.
...
function buildBedroom() {
  ...
  // North wall around bedroom window.
  ...
  // Shared west wall around doorway.
...
function addHeroBlockout() {
  ...
  round.rotation.x = Math.PI / 2; round.position.set(-.45, -2.05, .018); round.receiveShadow = true; round.name='PROXY__round-rug'; scene.add(round);
...
function addStovePipe() {
  const points = [
    new THREE.Vector3(1.95,.72,1.18), new THREE.Vector3(1.95,.72,2.28),
    new THREE.Vector3(1.35,.72,2.48), new THREE.Vector3(-2.15,.72,2.48), new THREE.Vector3(-2.55,.72,2.38)
  ];
  const curve = new THREE.CatmullRomCurve3(points, false, 'centripetal');
  const mesh = new THREE.Mesh(new THREE.TubeGeometry(curve, 72, .115, 12, false), MATERIALS.metal);
  mesh.name='blockout-stove-pipe'; mesh.castShadow=true; scene.add(mesh);
}

function addLighting() {
  scene.add(new THREE.HemisphereLight(0xb9d5e9, 0x4e392c, 1.15));
  const sun = new THREE.DirectionalLight(0xffe1ba, 2.0); sun.position.set(-5,-4,7); sun.castShadow=true; sun.shadow.mapSize.set(2048,2048); scene.add(sun);
  const stove = new THREE.PointLight(0xff6b25, 7.5, 3.2, 2); stove.position.set(1.95,.25,.62); scene.add(stove);
  const bedroom = new THREE.PointLight(0xffb45a, 4.5, 3.5, 2); bedroom.position.set(4.55,2.15,1.35); scene.add(bedroom);
}

class GhostObserver {
  constructor(camera, domElement) {
    this.camera=camera; this.domElement=domElement; this.keys=new Set(); this.yaw=-.18; this.pitch=-.08; this.speed=2.6;
    this.reset();
    domElement.addEventListener('click',()=>domElement.requestPointerLock?.());
    document.addEventListener('mousemove',e=>{if(document.pointerLockElement!==domElement)return;this.yaw-=e.movementX*.002;this.pitch-=e.movementY*.002;this.pitch=Math.max(-1.5,Math.min(1.5,this.pitch));});
    addEventListener('keydown',e=>{this.keys.add(e.code);if(e.code==='KeyR')this.reset();});
    addEventListener('keyup',e=>this.keys.delete(e.code));
  }
  reset(){this.camera.position.set(-1.4,-1.6,1.55);this.yaw=-.18;this.pitch=-.08;}
  update(dt){
    this.camera.rotation.order='ZYX'; this.camera.rotation.set(this.pitch,0,this.yaw);
    const forward=new THREE.Vector3(0,0,-1).applyQuaternion(this.camera.quaternion); forward.z=0; forward.normalize();
    const right=new THREE.Vector3().crossVectors(forward,new THREE.Vector3(0,0,1)).normalize().negate();
    const boost=this.keys.has('ShiftLeft')||this.keys.has('ShiftRight')?3:1; const s=this.speed*boost*dt;
    if(this.keys.has('KeyW'))this.camera.position.addScaledVector(forward,s); if(this.keys.has('KeyS'))this.camera.position.addScaledVector(forward,-s);
    if(this.keys.has('KeyA'))this.camera.position.addScaledVector(right,-s); if(this.keys.has('KeyD'))this.camera.position.addScaledVector(right,s);
    if(this.keys.has('KeyE'))this.camera.position.z+=s; if(this.keys.has('KeyQ'))this.camera.position.z-=s;
  }
}

try {
  buildMainRoom(); buildBedroom(); addHeroBlockout(); addStovePipe(); addLighting();
  const observer = new GhostObserver(camera, canvas);
  const clock = new THREE.Clock();
  renderer.setAnimationLoop(()=>{observer.update(Math.min(clock.getDelta(),.05));renderer.render(scene,camera);});
} catch (error) {
  console.error(error); fatal.hidden=false; fatal.textContent=`Fly House failed to initialize: ${error?.stack||error}`;
}
