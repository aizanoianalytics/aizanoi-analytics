(() => {
  ...
  function safeSpreadsheetCell(value){
    ...
    // Excel/LibreOffice may execute cells beginning with formula-control
    // characters even when quoted in CSV. Prefix a literal apostrophe.
    return /^[=+\-@]/.test(text.replace(/^\s+/,'')) ? `'${text}` : text;
  }

  function exportCSV(record,data){
    const quote=(value)=>`"${safeSpreadsheetCell(value).replace(/"/g,'""')}"`;
    const csv=[
      data.headers.map(quote).join(','),
      ...data.rows.map((row)=>data.headers.map((_,i)=>quote(row[i])).join(','))
    ].join('\n');
  ...
  async function render(win,payload={}){
    ...
    body.innerHTML=`<div class="az-lab-shell"><header class="az-workbench-toolbar az-lab-header"><div><span class="az-kicker">DATA LAB / LOCAL ANALYSIS</span><h2>${esc(record?.name||'No dataset open')}</h2></div><div class="az-toolbar-spacer"></div>${record?'<button class="az-tool-btn" data-lab-action="notes">Send to Notes</button><button class="az-tool-btn primary" data-lab-action="export">Export CSV</button>':'<button class="az-tool-btn primary" data-lab-action="archive">Open Field Archive</button>'}</header>${record?`<section class="az-metric-strip"><article><span>ROWS</span><b>${stats.rows}</b><small>loaded locally</small></article><article><span>COLUMNS</span><b>${stats.columns}</b><small>${stats.numericColumns} numeric-like</small></article><article><span>MISSING</span><b>${stats.missing}</b><small>${stats.total?(stats.missing/stats.total*100).toFixed(1):'0.0'}% cells</small></article><article><span>SIZE</span><b>${P.formatBytes(record.size||0)}</b><small>${esc(record.collection)}</small></article></section><div class="az-data-controls"><label class="az-search-field"><span>⌕</span><input data-lab-filter placeholder="Filter visible rows"></label><span>Preview capped at 500 rows. Analysis stays in this browser.</span></div><div data-lab-table>${preview(data)}</div>`:'<div class="az-empty-state az-lab-empty"><div class="az-empty-orbit">∑</div><h3>Data Lab is ready</h3><p>Open a CSV or JSON file from Field Archive to profile and inspect it locally.</p><button class="az-tool-btn primary" data-lab-action="archive">Open Field Archive</button></div>'}</div>`;
    body.onclick=(event)=>{const action=event.target.closest('[data-lab-action]')?.dataset.labAction;if(action==='archive')W.open('archive',{collection:'Datasets'});if(action==='notes'&&record)W.sendToNotes(record,summary(record,data,stats));if(action==='export'&&record)exportCSV(record,data);};
