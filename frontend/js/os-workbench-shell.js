(() => {
  ...
  function commands(){
    ...
    if(context.appId==='archive'){add('Import files','Add local research material',()=>W.existingWindow('archive')?.querySelector('[data-archive-file-input]')?.click(),'/assets/icons/field-archive.svg');add('New Field Note','Create an archived note',async()=>{const note=await A.createNote();W.open('notes',{recordId:note.id});},'/assets/icons/notepad.svg');add('Workspace Monitor','Storage, backend and session status',()=>W.open('monitor'),'/assets/icons/workspace-monitor.svg');}
