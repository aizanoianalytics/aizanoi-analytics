(() => {
  ...
  function featuredApps(){return S.apps.filter((app)=>app.featured!==false);}
  ...
  function commands(){
    ...
    if(context.appId==='archive'){add('Import files','Add local research material',()=>W.existingWindow('archive')?.querySelector('[data-archive-file-input]')?.click(),'/assets/icons/field-archive.svg');add('New Field Note','Create an archived note',async()=>{const note=await A.createNote();W.open('notes',{recordId:note.id});},'/assets/icons/notepad.svg');add('Workspace Monitor','Storage, API and session status',()=>W.open('monitor'),'/assets/icons/workspace-monitor.svg');}
  ...
  async function restoreSession(){await new Promise((resolve)=>setTimeout(resolve,950));const prefs=S.getState();if(!prefs.restoreSession||matchMedia('(max-width:700px)').matches)return;for(const id of prefs.sessionApps.filter((id)=>W.apps.includes(id)).slice(0,3))if(!W.existingWindow(id))await W.open(id);}
