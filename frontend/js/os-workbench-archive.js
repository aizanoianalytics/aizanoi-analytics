(() => {
  ...
  async function showFileMenu(record,x,y){
    ...
    menu.innerHTML=`<button data-action="open">Open <span>↗</span></button><button data-action="preview">Quick Look <span>Space</span></button><div></div><button data-action="notes">Send to Field Notes</button>${!['image','file'].includes(record.kind)?'<button data-action="ai">Ask Aizanoi AI</button>':''}<div></div><button data-action="rename">Rename</button><button class="danger" data-action="delete">Delete</button>`;document.body.appendChild(menu);
    menu.onclick=async(event)=>{const action=event.target.closest('[data-action]')?.dataset.action;if(!action)return;menu.remove();if(action==='open')W.openRecord(record);if(action==='preview')W.quickLook(record);if(action==='notes')W.sendToNotes(record);if(action==='ai')W.askAI('Explain and assess this archive item. Pull out the most useful points and suggest what I should inspect next.',`Archive item: ${record.name}\nType: ${record.kind}\nCollection: ${record.collection}\n\n${String(record.text||'').slice(0,14000)}`);if(action==='rename'){const next=prompt('Rename archive item:',record.name);if(next){await A.rename(record.id,next);W.refresh('archive');}}if(action==='delete'&&confirm(`Delete "${record.name}" from this browser?`)){await A.remove(record.id);selectedFile=null;W.refresh('archive');}};
