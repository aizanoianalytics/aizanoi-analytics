...
function rewriteDock(api){
  ...
  if(running)new MutationObserver(()=>queueMicrotask(()=>syncPinned(api))).observe(running,{childList:true,subtree:true});
}
...
export function installBrandPlatform(api){
  ...
  rewriteDock(api);
  watchLauncher();
  api.store.subscribe(()=>syncPinned(api));
