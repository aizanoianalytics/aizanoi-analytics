...
export async function mount({container,api}){
  container.innerHTML=`<div class="az-app-shell"><div class="az-app-toolbar"><strong>Aizanoi TV</strong><span class="az-system-spacer"></span><span style="color:var(--az-text-3);font-size:11px">Walkthroughs & field stories</span></div><div class="az-media"><div class="az-simple-grid">${ITEMS.map((item)=>`<article class="az-simple-card"><p class="az-kicker">${esc(item.type)}</p><h3>${esc(item.title)}</h3><p>${esc(item.body)}</p><button class="az-button" type="button" data-media-action="${esc(item.action)}">${esc(item.label)}</button></article>`).join('')}</div><div class="az-simple-card" style="margin-top:10px"><h3>Video layer</h3><p>External video embeds are intentionally not loaded on app open. A future curated walkthrough can use privacy-enhanced embeds with transcript, chapters and linked landmarks.</p></div></div></div>`;
