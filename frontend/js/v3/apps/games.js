const GAMES={
  ...
  blockfall:{label:'Blockfall',description:'Stack falling blocks and clear lines',container:'game-blockfall-container',script:'/games/blockfall.js',mount:'AizanoiArcadeBlocks'}
};
...
export async function mount({container,api}){
  let active=null;let scriptNode=null;let gameCleanup=null;
  ...
  function teardownGame(){try{gameCleanup?.();}catch(_){ } gameCleanup=null;}
  async function play(id){
    const game=GAMES[id];if(!game)return;active=id;
    teardownGame();
    ...
    await ensureUtils();
    scriptNode?.remove();scriptNode=await loadScript(game.script);
    // Modular games expose a mount factory on window (GAMES[].mount key);
    // legacy self-initializing scripts keep working untouched.
    ...
    if(factory&&typeof factory.mount==='function'){
      ...
      gameCleanup=factory.mount(host,()=>{active=null;stage.innerHTML='';teardownGame();scriptNode?.remove();scriptNode=null;})||null;
    }
  }
  const closeGame=()=>{active=null;stage.innerHTML='';teardownGame();scriptNode?.remove();scriptNode=null;};
  const click=(event)=>{const id=event.target.closest('[data-play-game]')?.dataset.playGame;if(id)play(id).catch((error)=>{teardownGame();stage.innerHTML=`<div class="az-empty-state"><div><h3>Game unavailable</h3><p>${esc(error.message)}</p></div></div>`;});if(event.target.closest('[data-close-game]'))closeGame();};
  container.addEventListener('click',click);
  return()=>{container.removeEventListener('click',click);closeGame();};
}
