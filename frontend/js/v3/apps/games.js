...
export async function mount({container}){
  ...
  container.innerHTML=`<div class="az-app-shell"><div class="az-app-toolbar"><strong>Experiments</strong><span class="az-system-spacer"></span><span class="az-app-caption">Local interaction playground · not historical evidence</span></div><div class="az-games"><div class="az-simple-grid">${Object.entries(GAMES).map(([id,game])=>`<article class="az-simple-card"><p class="az-kicker">EXPERIMENT</p><h3>${esc(game.label)}</h3><p>${esc(game.description)}</p><button class="az-button" type="button" data-play-game="${id}">Play</button></article>`).join('')}</div><div class="az-game-stage" data-game-stage></div></div></div>`;
