...
export async function mount({container,api}) {
  let stopped=false;
  async function draw(){
    if(stopped)return;const estimate=await storageEstimate();const session=getFieldSession();let sw='Not registered';
    ...
    container.innerHTML=`<div class="az-app-shell"><div class="az-app-toolbar"><strong>Workspace Monitor</strong><span class="az-system-spacer"></span><span class="az-app-caption">Measured from this browser · ${new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</span></div><div class="az-monitor"><div class="az-monitor-grid">${cards.map(([label,value,detail])=>`<article class="az-monitor-card"><span>${esc(label)}</span><strong>${esc(value)}</strong><p>${esc(detail)}</p></article>`).join('')}</div></div></div>`;
  }
