...
export async function mount({container,api}) {
  ...
  container.innerHTML=`<div class="az-app-shell"><div class="az-app-toolbar"><strong>Historical Worlds</strong><span class="az-system-spacer"></span>${session?`<button class="az-button az-button-primary" type="button" data-continue-world>Continue ${escapeHtml(session.worldId)}</button>`:''}</div><div class="az-projects"><div class="az-project-grid">${WORLDS.map((world)=>`<article class="az-project-card"><p class="az-kicker">${escapeHtml(world.era)}</p><h3>${escapeHtml(world.label)}</h3><p>${escapeHtml(world.summary)}</p><p class="az-reading-list-meta">${escapeHtml(world.evidence)} · ${escapeHtml(world.duration)}</p><div class="az-project-actions"><button class="az-button az-button-primary" type="button" data-open-world="${escapeHtml(world.id)}">Enter world</button></div></article>`).join('')}</div></div></div>`;
