/** Aizanoi Arcade — Blockfall (tetromino stacking, original implementation). */
(function () {
  'use strict';
  const SHAPES = [
  ...
  function mount(container, onClose) {
    ...
    function rotate() {
      ...
      else if (!collide(piece.x - 1, piece.y, rotated)) { piece.x--; piece.cells = rotated; }
      else if (!collide(piece.x + 1, piece.y, rotated)) { piece.x++; piece.cells = rotated; }
    }
    function merge() { piece.cells.forEach((row,y)=>row.forEach((value,x)=>{if(value&&piece.y+y>=0)grid[piece.y+y][piece.x+x]=piece.color;})); }
    function sweep() {
      let cleared = 0;
      ...
      if (cleared) {
        ...
        scoreEl.textContent = Math.floor(score); linesEl.textContent = lines;
      }
    }
    function drop() { if (!collide(piece.x,piece.y+1,piece.cells)) piece.y++; else { merge(); sweep(); spawn(); } dropCounter=0; }
    function hardDrop() { while (!collide(piece.x,piece.y+1,piece.cells)) { piece.y++; score++; } drop(); scoreEl.textContent=Math.floor(score); }
    function drawCell(context,x,y,color,size=CELL){context.fillStyle=color;context.fillRect(x*size+1,y*size+1,size-2,size-2);context.fillStyle='rgba(255,255,255,.25)';context.fillRect(x*size+1,y*size+1,size-2,3);}
    function drawNext(){nctx.clearRect(0,0,nextCanvas.width,nextCanvas.height);next?.cells.forEach((row,y)=>row.forEach((value,x)=>{if(value)drawCell(nctx,x,y,next.color);}));}
    function draw(){
      ctx.fillStyle='#0d1524';ctx.fillRect(0,0,canvas.width,canvas.height);
      ...
      if(!running){ctx.fillStyle='rgba(6,10,18,.72)';ctx.fillRect(0,0,canvas.width,canvas.height);ctx.fillStyle='#7ff0c8';ctx.font='700 22px system-ui';ctx.textAlign='center';ctx.fillText('Game over',canvas.width/2,canvas.height/2-8);ctx.font='13px system-ui';ctx.fillStyle='#9fb0c8';ctx.fillText('Press Restart',canvas.width/2,canvas.height/2+18);}
    }
    function loop(timestamp=0){const delta=timestamp-(loop.last||timestamp);loop.last=timestamp;if(running){dropCounter+=delta;if(dropCounter>dropInterval)drop();draw();}raf=requestAnimationFrame(loop);}
    function restart(){grid=emptyGrid();score=0;lines=0;dropCounter=0;dropInterval=620;running=true;next=null;spawn();scoreEl.textContent='0';linesEl.textContent='0';}
    ...
    container.querySelector('[data-bf-pause]').addEventListener('click', (event) => { running=!running; event.target.textContent=running?'Pause':'Resume'; });
    const touch=(event)=>{
      const button=event.target.closest('[data-bf-left],[data-bf-right],[data-bf-rotate],[data-bf-down],[data-bf-drop]');if(!button||!running)return;event.preventDefault();
