(function() {
  ...
  let paddleX,ballX,ballY,ballDX,ballDY,bricks,score,lives,over,particles=[],trail=[],paused=false;
  let rafId=null,lastFrame=0,accumulator=0;

  function stopLoop(){if(rafId!==null)cancelAnimationFrame(rafId);rafId=null;lastFrame=0;accumulator=0;}
  function startLoop(){stopLoop();rafId=requestAnimationFrame(frame);}
  function frame(now){
    if(!container.isConnected){stopLoop();return;}
    if(!lastFrame)lastFrame=now;
    const elapsed=Math.min(80,Math.max(0,now-lastFrame));lastFrame=now;
    if(!paused&&!over){accumulator+=elapsed;let steps=0;while(accumulator>=FRAME_MS&&steps<4&&!over){tickStep();accumulator-=FRAME_MS;steps++;}if(steps===4)accumulator=0;}
    draw();
    if(!over&&container.isConnected)rafId=requestAnimationFrame(frame);else rafId=null;
  }

  function init(){
    paddleX=(W-PADDLE_W)/2;ballX=W/2;ballY=H-38;ballDX=3;ballDY=-3;score=0;lives=3;over=false;paused=false;particles=[];trail=[];stopLoop();
    bricks=[];const rows=5,cols=8,bw=(W-24)/cols;const palette=[['#ff7f76','#bb2535'],['#ffc15f','#db6f2f'],['#79d77f','#2d8b57'],['#61b9f6','#2a62bc'],['#aa86e8','#5c42a4']];
    ...
    const shell=document.createElement('div');shell.style.cssText='width:min(100%,420px);background:#09101d;border:3px solid #445873;border-radius:8px;padding:8px;box-shadow:inset 0 0 0 2px #111c2b,0 5px 14px rgba(0,0,0,.18);';
    const bar=document.createElement('div');bar.style.cssText='height:30px;display:flex;align-items:center;justify-content:space-between;padding:0 5px;color:#dfeeff;font:700 11px Tahoma,sans-serif;letter-spacing:.05em;';bar.innerHTML='<span>AIZANOI BREAKOUT</span><span><span id="brick-score">0000</span> · ♥ <span id="brick-lives">3</span></span>';shell.appendChild(bar);
    const canvas=document.createElement('canvas');canvas.width=W;canvas.height=H;canvas.id='brick-canvas';canvas.tabIndex=0;canvas.setAttribute('aria-label','Brick Breaker game. Move with arrow keys, mouse or touch.');canvas.style.cssText='width:100%;height:auto;aspect-ratio:4/3;background:#050a12;display:block;border:1px solid #4c6079;border-radius:3px;touch-action:none;';shell.appendChild(canvas);container.appendChild(shell);
  ...
  function tickStep(){
    particles.forEach(p=>{p.x+=p.vx;p.y+=p.vy;p.vy+=.08;p.life--;});particles=particles.filter(p=>p.life>0);trail.push({x:ballX,y:ballY});if(trail.length>7)trail.shift();ballX+=ballDX;ballY+=ballDY;
    ...
    if(bricks.every(b=>!b.alive)){end(true);return;}if(ballY>H+BALL_R){lives--;if(lives<=0){end(false);return;}resetBall();}
  }

  function roundRect(ctx,x,y,w,h,r){ctx.beginPath();ctx.roundRect?ctx.roundRect(x,y,w,h,r):ctx.rect(x,y,w,h);ctx.fill();}
  function draw(){
    const c=document.getElementById('brick-canvas');if(!c)return;const ctx=c.getContext('2d');const bg=ctx.createLinearGradient(0,0,0,H);bg.addColorStop(0,'#07172a');bg.addColorStop(1,'#02060b');ctx.fillStyle=bg;ctx.fillRect(0,0,W,H);
  ...
  function togglePause(){if(over)return;paused=!paused;const button=container.querySelector('[data-game-action="pause"]');if(button){button.textContent=paused?'Resume':'Pause';button.setAttribute('aria-pressed',paused?'true':'false');}if(!paused&&!rafId)startLoop();draw();}
  function end(won){over=true;saveScore('brick',score);window.AizanoiGames?.refreshToolbar(container,'brick');if(won)burst(W/2,H/2,'#ffd36b');draw();stopLoop();}
