...
export async function mount({container,api}) {
  ...
  container.innerHTML=`<div class="az-app-shell"><div class="az-app-toolbar"><strong>Projects</strong><span class="az-system-spacer"></span><span class="az-app-caption">Current public work · no retired product surfaces</span></div><div class="az-projects"><div class="az-project-grid">${PROJECTS.map((project)=>`<article class="az-project-card"><p class="az-kicker">${escapeHtml(project.kicker)}</p><h3>${escapeHtml(project.label)}</h3><p>${escapeHtml(project.body)}</p><p class="az-status-accent">${escapeHtml(project.status)}</p><div class="az-project-actions"><button class="az-button az-button-primary" type="button" data-project-app="${escapeHtml(project.action)}">${escapeHtml(project.actionLabel)}</button></div></article>`).join('')}</div>${session?`<section class="az-simple-card az-project-session"><p class="az-kicker">RECENT FIELD SESSION</p><h3>${escapeHtml(WORLDS.find((world)=>world.id===session.worldId)?.label||session.worldId)}</h3><p>Your most recent historical-world context is stored locally and can be resumed from Home or the World Index.</p><button class="az-button" type="button" data-continue>Continue session</button></section>`:''}</div></div>`;
