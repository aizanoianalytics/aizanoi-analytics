(() => {
  ...
  async function renderSource(win,payload={}){
    ...
    body.innerHTML=`<div class="az-reader-shell"><header class="az-workbench-toolbar"><div><span class="az-kicker">SOURCE READER / LOCAL ARCHIVE</span><h2>${esc(record?.name||'No source open')}</h2></div><div class="az-toolbar-spacer"></div>${record?'<label class="az-search-field compact"><span>⌕</span><input data-source-search placeholder="Find in source"></label><button class="az-tool-btn" data-source-action="notes">Send to Notes</button>':'<button class="az-tool-btn primary" data-source-action="archive">Open Archive</button>'}</header><section class="az-reader-canvas" data-source-canvas>${record?'<div class="az-preview-loading">Opening source…</div>':'<div class="az-empty-state"><div class="az-empty-orbit">¶</div><h3>Source Reader</h3><p>Open PDF, Markdown or text material from Field Archive. Content stays in this browser.</p></div>'}</section>${record?`<footer class="az-workbench-status"><span>${esc(record.collection)}</span><span>${P.formatBytes(record.size||0)}</span><span data-source-match>LOCAL ONLY</span></footer>`:''}</div>`;
    if(record){
      ...
      else {const text=record.text||'';canvas.innerHTML=`<article class="az-source-document" data-source-document>${['markdown','note'].includes(record.kind)?renderMarkdown(text):`<pre>${esc(text)}</pre>`}</article>`;const search=$('[data-source-search]',body);search?.addEventListener('input',()=>{const q=search.value.trim().toLowerCase(),count=q?(text.toLowerCase().match(new RegExp(W.escapeRegExp(q),'g'))||[]).length:0;$('[data-source-match]',body).textContent=q?`${count} MATCH${count===1?'':'ES'}`:'LOCAL ONLY';});}
    }
    body.onclick=async(event)=>{const action=event.target.closest('[data-source-action]')?.dataset.sourceAction;if(action==='archive')W.open('archive',{collection:'Sources'});if(action==='notes'&&record)W.sendToNotes(record);};
  }
