(function() {
  ...
  let paddleX,ballX,ballY,ballDX,ballDY,bricks,score,lives,over,interval,particles=[],trail=[];

  function init(){
    paddleX=(W-PADDLE_W)/2;ballX=W/2;ballY=H-38;ballDX=3;ballDY=-3;score=0;lives=3;over=false;particles=[];trail=[];
  ...
  function onKey(e){if(e.key==='ArrowLeft'){e.preventDefault();paddleX=Math.max(0,paddleX-22);}if(e.key==='ArrowRight'){e.preventDefault();paddleX=Math.min(W-PADDLE_W,paddleX+22);}if((e.key===' '||e.key==='Enter')&&over){e.preventDefault();init();}}
  ...
  function draw(){
    ...
    const s=document.getElementById('brick-score'),l=document.getElementById('brick-lives'),st=document.getElementById('brick-status');if(s)s.textContent=String(score).padStart(4,'0');if(l)l.textContent=lives;if(st)st.textContent=over?'Round complete · Score: '+score:'Mouse, touch or arrows move the paddle · Clear every signal block';
  }

  function end(won){over=true;if(interval)clearInterval(interval);interval=null;saveScore('brick',score);if(won)burst(W/2,H/2,'#ffd36b');draw();}
  function saveScore(game,score){try{const key='aizanoi-games',scores=JSON.parse(localStorage.getItem(key)||'{}');if(!scores[game])scores[game]=[];scores[game].push({score,at:new Date().toISOString()});if(scores[game].length>20)scores[game]=scores[game].slice(-20);localStorage.setItem(key,JSON.stringify(scores));}catch(_){}}
  init();
})();