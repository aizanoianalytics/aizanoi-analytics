...
function chart() {
  ...
  const low = Math.min(...closes,...overlays); const high = Math.max(...closes,...overlays); const spread = high - low || 1;
  ...
  const price = `<figure><figcaption>Close price · ${esc(start)}–${esc(end)} · ${number.format(selected.length)} observations</figcaption><svg class="price-chart" viewBox="0 0 900 360" role="img" aria-label="Close-price chart with selected overlays"><g transform="translate(0 10)">${line(closes,'i-price',900,340,low,spread)}${overlayLines}</g></svg></figure>`;
  const oscillatorCharts = [...state.oscillators].map(key => {
    ...
    const minimum = key === 'rsi14' ? 0 : Math.min(...values); const maximum = key === 'rsi14' ? 100 : Math.max(...values); const range = maximum - minimum || 1;
    const lines = keys.map(name => line(sliced[name], `i-${name}`,900,130,minimum,range)).join('');
    return `<figure><figcaption>${key === 'rsi14' ? 'RSI 14' : key === 'macd' ? 'MACD 12/26/9' : 'Rate of change 20'}</figcaption><svg class="oscillator-chart" viewBox="0 0 900 150" role="img" aria-label="${esc(key)} oscillator"><g transform="translate(0 10)">${lines}</g></svg></figure>`;
  }).join('');
...
function render() {
  ...
  document.querySelector('[data-instrument-meta]').textContent = `${state.payload.exchange} · Daily history from ${state.payload.startDate || 'first available session'} · ${state.payload.fourHour?.length ? 'Recent 4H history available' : '4H unavailable from source'}`;
  ...
  document.querySelector('[data-instrument-quality]').innerHTML = `<strong>Data quality</strong><p>Daily close: ${esc(quality.dailyClose || 'unknown')} · Recent 4H: ${esc(quality.fourHour || 'unavailable')} · History: ${esc(quality.history || 'unknown')} · Published model: close-only.</p>`;
}
function searchResults(value) {
  ...
  target.innerHTML = matches.map(row => `<a href="${detailUrl(market,row.slug)}"><strong>${esc(row.ticker)}</strong> ${esc(row.name)}</a>`).join('');
}

document.addEventListener('change', event => {
  if (event.target.matches('[data-frequency]')) state.frequency = event.target.value;
  if (event.target.matches('[data-timeframe]')) state.timeframe = event.target.value;
...
document.querySelector('[data-instrument-search]').addEventListener('submit', event => { event.preventDefault(); const needle = document.querySelector('[data-symbol-search]').value.trim().toLowerCase(); const exact = state.universe.find(row => row.ticker.toLowerCase() === needle || row.slug === needle); if (exact) location.href = detailUrl(market, exact.slug); });
