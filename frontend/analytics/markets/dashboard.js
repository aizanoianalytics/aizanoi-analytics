import { createWatchlist, detailUrl, filterRows, formatLevelPercent, formatPercent, marketSessionState, rowsToCsv, sortRows } from './core.js';
...
function sparkPoints(values) {
  ...
  const low = Math.min(...safe); const spread = Math.max(...safe) - low || 1;
...
function sparkline(values) {
  ...
  if (finite.length < 2) return '<span class="market-no-data">—</span>';
  return `<svg class="market-mini-spark" viewBox="0 0 90 32" preserveAspectRatio="none" aria-hidden="true"><polyline points="${sparkPoints(finite)}"/></svg>`;
}
...
export function createMarketsDashboard(container, { compact = false, updateUrl = !compact } = {}) {
  ...
  function syncExplorerChrome() {
    const currentMarketRows = currentRows();
    if (state.market === 'us' && currentMarketRows.length > 250) query('[data-row-cap-note]').hidden = false;
  }
  ...
  function renderHealth() {
    ...
    query('[data-health]').innerHTML = card('Pipeline', health.status || state.manifest?.status || '—') + card('US instruments', number.format(health.counts?.us || 0)) + card('Crypto assets', number.format(health.counts?.crypto || 0)) + card('Failed symbols', number.format(health.failedSymbols?.length || 0)) + card('Daily-only instruments', number.format(quality.fourHourUnavailable || 0)) + card('Limited history', number.format(quality.limitedHistory || 0)) + card('Published model', 'Close-only') + card('Last refresh', health.completedAt ? new Date(health.completedAt).toLocaleString() : '—');
  }
  ...
  const input = event => { if (event.target.matches('[data-search]')) { state.query = event.target.value.trim().toLowerCase(); renderExplorer(); updateLocation(); } };
  ...
  Promise.all([getJson(`${DATA_ROOT}/manifest.json`, controller.signal), getJson(`${DATA_ROOT}/health.json`, controller.signal).catch(() => null), getJson(`${DATA_ROOT}/snapshots/pulse.json`, controller.signal).catch(() => ({ snapshots:[] }))]).then(([manifest,health,snapshots]) => { state.manifest = manifest; state.health = health; state.snapshots = snapshots.snapshots || []; query('[data-freshness]').textContent = new Date(manifest.completedAt).toLocaleString(); return loadMarket(state.market); }).catch(showError);
  syncControls();
  return () => { controller.abort(); container.replaceChildren(); };
}
