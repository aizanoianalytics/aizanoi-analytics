...
const PRECACHE=['/','/release.js','/manifest.webmanifest','/assets/branding/aizanoi-logo-mark.svg','/assets/branding/aizanoi-pwa-192.png','/assets/branding/aizanoi-pwa-512.png','/assets/branding/aizanoi-pwa-maskable-512.png','/assets/wallpapers/aizanoi-os-sunrise.svg','/styles/tokens.css','/styles/base.css','/styles/shell.css','/styles/components.css','/styles/device-shell.css','/styles/polish.css','/styles/utility-polish.css','/js/v3/main.js','/js/v3/aizanoi-os.js','/js/v3/brand-platform.js','/js/v3/registry.js','/js/v3/module-registry.generated.js','/js/v3/store.js','/js/v3/shell.js','/news/index.json'];
...
self.addEventListener('activate',(event)=>event.waitUntil(caches.keys().then((keys)=>Promise.all(keys.filter((key)=>(key.startsWith('aizanoi-field-shell-')||key.startsWith('aizanoi-os-shell-'))&&key!==CACHE).map((key)=>caches.delete(key)))).then(()=>self.clients.claim())));
function sameOriginStatic(request){const url=new URL(request.url);return request.method==='GET'&&url.origin===self.location.origin&&/\.(?:css|js|mjs|json|svg|png|jpg|jpeg|webp|gif|woff2?)$/i.test(url.pathname);}
function requestCacheKey(request){const url=new URL(request.url);return`${url.pathname}${url.search}`;}
async function pruneRuntimeCache(){const cache=await caches.open(CACHE),keys=await cache.keys(),runtime=keys.filter((key)=>!PRECACHE_KEYS.has(requestCacheKey(key))),excess=runtime.length-MAX_RUNTIME_ENTRIES;if(excess<=0)return;for(const key of runtime.slice(0,excess))await cache.delete(key);}
