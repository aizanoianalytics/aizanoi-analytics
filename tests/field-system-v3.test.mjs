...
assert.equal(registry.APPS.length, 17, 'public AizanoiOS catalog must contain the eight product families, market intelligence, plus the eight workspace utilities');
assert.equal(registry.ALL_APPS.length, 17, 'combined public app catalog must match the registry exactly');
...
assert.match(sw, /self\.addEventListener\('install',\s*\(event\)\s*=>\s*event\.waitUntil\(precacheShell\(\)\.then\(\(\)\s*=>\s*self\.skipWaiting\(\)\)\)\)/, 'service worker install must precache completely before force-activating the release');
assert.match(sw, /self\.clients\.claim\(\)/, 'activated release must immediately claim open AizanoiOS clients');
