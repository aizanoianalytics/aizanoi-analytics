...
for(const spec of worlds){
  ...
  const before=await position(page);await page.keyboard.down('KeyW');await page.waitForTimeout(450);await page.keyboard.up('KeyW');await page.waitForTimeout(80);const after=await position(page);const d=Math.hypot(after.x-before.x,after.z-before.z);assert.ok(d>0.05&&d<20,`${spec.id}: WASD movement unstable (${d})`);
