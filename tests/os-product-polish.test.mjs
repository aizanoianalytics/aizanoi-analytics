...
test('AI responses use safe markdown plus multiline abort retry and copy/clear UX', () => {
  ...
  assert.match(chat, /event\.isComposing|e\.isComposing/);
  assert.match(osJs, /Retry last/);
  assert.match(osJs, /aria-live/);
  assert.doesNotMatch(osJs, /installChatFetchGuard|ensureChatTextarea|CHAT_TIMEOUT_MS/);
});

test('core window lifecycle owns document listener cleanup while V2 owns recovery', () => {
  ...
  assert.match(index, /__AIZANOI_OS_V2__\?\.clampWindows/);
  assert.match(osJs, /taskbarTop/);
  assert.match(osJs, /document\.addEventListener\('mouseup'/);
  assert.doesNotMatch(osJs, /installWindowLifecycleBridge|detachDrag/);
});
...
test('desktop enhancement includes recoverable marquee selection on fine pointers', () => {
  assert.match(osJs, /installDesktopMarquee/);
  assert.match(osJs, /pointer:coarse/);
  assert.match(osJs, /os-v2-marquee/);
});
...
test('modular frontend respects tighter file-size budgets', () => {
  ...
  assert.ok(statSync('frontend/css/os-core.css').size < 55_000, 'os-core.css too large');
  assert.ok(statSync('frontend/js/chat.js').size < 10_000, 'chat.js too large');
  assert.ok(statSync('frontend/js/terminal.js').size < 14_000, 'terminal.js too large');
  assert.ok(statSync('frontend/js/os-router.js').size < 6_000, 'os-router.js too large');
  assert.ok(statSync('frontend/js/os-v2.js').size < 20_000, 'os-v2.js too large');
