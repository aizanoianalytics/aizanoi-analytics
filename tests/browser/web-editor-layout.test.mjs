...
test('Web Editor streams preview console output without relaxing the opaque sandbox',async()=>{
  ...
  try{
    ...
    await source.fill(`<!doctype html><html><body><h1>Console test</h1><script>
      ...
      setTimeout(()=>{ Promise.reject(new Error('handled-boom')).catch(()=>console.log('handled rejection consumed')); },90);
    <\/script></body></html>`);
    ...
    await output.getByText(/handled rejection consumed/i).waitFor({state:'visible',timeout:5000});
    assert.equal(await output.getByText(/Unhandled rejection:.*handled-boom/i).count(),0,'handled Promise.reject calls must not be reported as unhandled');
