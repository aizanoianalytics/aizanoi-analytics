...
test('Forge is removed from the shared brand hub implementation', () => {
  assert.doesNotMatch(legacyHubs, /mountForge|Aizanoi Forge|SOURCE OF TRUTH|aizanoianalytics\/aizanoi-analytics/);
  assert.match(legacyHubs, /mountJournal/);
  assert.match(legacyHubs, /mountLabs/);
});
