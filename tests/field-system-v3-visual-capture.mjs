...
await capture('aizanoi-os-08-workbench',{width:1440,height:900},async(page)=>{await page.evaluate(()=>window.AIZANOI_OS.openApp('workbench'));await page.waitForSelector('.az-window[data-app-id="workbench"]');});
await capture('aizanoi-os-09-tablet',{width:900,height:1180});
await capture('aizanoi-os-10-tablet-news',{width:900,height:1180},async(page)=>{await page.evaluate(()=>window.AIZANOI_OS.openApp('news'));await page.waitForSelector('.az-window[data-app-id="news"]');});
await capture('aizanoi-os-11-mobile',{width:390,height:844});
await capture('aizanoi-os-12-mobile-news',{width:390,height:844},async(page)=>{await page.evaluate(()=>window.AIZANOI_OS.openApp('news'));await page.waitForSelector('.az-window[data-app-id="news"]');});
await capture('aizanoi-os-13-mobile-command',{width:390,height:844},async(page)=>{await page.locator('[data-shell-action="search"]').first().click();await page.waitForSelector('#az-command-overlay.is-open');});
