...
test('public allowlist: every frontend/* file is a public web asset', () => {
  ...
  const offenders = files.map((p) => path.relative(repoRoot, p)).filter((rel) => !rel.startsWith('frontend' + path.sep));
...
const ALLOWED_DOWNLOADS = [/frontend[\\/]analytics[\\/]dashboards[\\/]hr-analytics-full-set[\\/]downloads[\\/]hr-analytics-full-set-synthetic-output\.xlsx$/];
const ALLOWED_JSON = [
...
test('denylist: no source or workbook files enter frontend except the declared synthetic output download', () => {
  const files = listDir(path.join(repoRoot, 'frontend'));
  const offenders = files.map((p) => path.relative(repoRoot, p)).filter((rel) => DENY_PATTERNS.some((re) => re.test(rel))).filter((rel) => !ALLOWED_DOWNLOADS.some((re) => re.test(rel)));
...
test('JSON allow-list: every JSON inside frontend/ is on the public list', () => {
  ...
  const offenders = files.map((p) => path.relative(repoRoot, p).replaceAll(path.sep, '/')).filter((rel) => !ALLOWED_JSON.some((re) => re.test(rel)));
...
test('deploy is cwd-independent: same frontend/ → staging result from different cwds', { skip: process.platform === 'win32' }, async () => {
  ...
  try {
    ...
    const a = list(stagingA), b = list(stagingB);
...
test('webroot smoke: all 11 dashboard HTML routes exist under frontend/', () => {
  const dashboards = [
    'analytics/dashboards/hr-analytics-full-set/index.html','analytics/dashboards/hr-analytics-full-set/corporate-goals/index.html','analytics/dashboards/hr-analytics-full-set/hr-administration-deep-dive/index.html','analytics/dashboards/hr-analytics-full-set/hr-executive-board-current/index.html','analytics/dashboards/hr-analytics-full-set/hr-executive-board-current/pdks_takip_dashboard.html','analytics/dashboards/hr-analytics-full-set/hr-executive-board-full-history/index.html','analytics/dashboards/hr-analytics-full-set/hr-executive-board-full-history/pdks_takip_dashboard.html','analytics/dashboards/hr-analytics-full-set/learning-academy-analytics/index.html','analytics/dashboards/hr-analytics-full-set/performance-hiring-turnover/index.html','analytics/dashboards/hr-analytics-full-set/store-learning-compliance/index.html','analytics/dashboards/hr-analytics-full-set/store-operations-tracking/index.html','analytics/dashboards/hr-analytics-full-set/workforce-time-attendance/index.html','analytics/dashboards/hr-analytics-full-set/workforce-turnover/index.html','analytics/dashboards/hr-analytics-full-set/downloads/hr-analytics-full-set-synthetic-output.xlsx'
  ];
...
test('legacy /analytics/workforce-turnover/ stays 404 (owner decision 2026-08-26)', () => {
  ...
  const hasFrontendCopy = existsSync(legacyFrontend), hasRepoRedirect = existsSync(legacyInRepo);
