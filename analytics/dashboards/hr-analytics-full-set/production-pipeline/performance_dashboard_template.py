...
function kpis(id,cards){$(id).innerHTML=cards.map(c=>`<article class="kpi"><div class="label">${esc(c.label)}</div><div class="value">${esc(c.value)}</div><div class="sub">${esc(c.sub)||''}</div></article>`).join('')}
...
function renderPoolChips(){$('poolChips').innerHTML=[...state.pool].map(i=>`<span class="chip">${esc(poolPeople[i]?.ad_soyad||poolPeople[i]?.sicil)}<button data-remove="${esc(i)}" type="button" aria-label="Havuzdan kaldır" title="Havuzdan kaldır">×</button></span>`).join('')||'<span class="hint">Henüz çalışan seçilmedi.</span>';$('poolChips').querySelectorAll('[data-remove]').forEach(b=>b.onclick=()=>{state.pool.delete(Number(b.dataset.remove));renderPoolSearch();renderPoolChips()});renderPoolPeopleTable()}
...
function renderTargetSummary(){
  ...
  $('targetDistribution').innerHTML=`<div class="target-status-strip">${order.map(k=>`<span style="width:${esc((counts[k]||0)/den*100)}%;background:${esc(TARGET_STATUS[k].color)}" title="${esc(k)}: ${esc(counts[k]||0)}"></span>`).join('')}</div><div class="target-status-grid">${order.map(k=>`<div class="target-status-item"><i style="background:${esc(TARGET_STATUS[k].color)}"></i><span>${esc(TARGET_STATUS[k].short)}</span><b>${num(counts[k]||0,0)}</b></div>`).join('')}</div>`;
}
