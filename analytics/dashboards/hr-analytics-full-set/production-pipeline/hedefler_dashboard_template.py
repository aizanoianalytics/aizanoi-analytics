...
function renderControls(){
  ...
  $('periodTabs').innerHTML=DATA.periods.map(p=>{const enabled=scope().available_periods.includes(p.key);return`<button class="period ${p.key===state.period?'active':''}" data-period="${esc(p.key)}" ${enabled?'':'disabled'} title="${esc(p.range)}">${esc(p.short_label)}</button>`}).join('');
...
function renderKpis(){
  ...
  $('kpiGrid').innerHTML=cards.map(c=>`<article class="kpi" style="--tone:${esc(c[3])}"><div class="label">${esc(c[0])}</div><div class="value">${esc(c[1])}</div><div class="sub">${esc(c[2])}</div></article>`).join('');
}
function renderDistribution(){
  ...
  $('statusDistribution').innerHTML=`<div class="status-strip">${order.map(k=>`<span style="width:${esc((counts[k]||0)/den*100)}%;background:${esc(STATUS[k].color)}" title="${esc(k)}: ${esc(counts[k]||0)}"></span>`).join('')}</div><div class="status-grid">${order.map(k=>`<div class="status-item"><i class="dot" style="background:${esc(STATUS[k].color)}"></i><span>${esc(STATUS[k].short)}</span><b>${num(counts[k]||0,0)}</b></div>`).join('')}</div>`;
}
